...
class DecoderLayer(nn.Module):
    ...
    def forward(self,x,enc_output,src_mask,tgt_mask):
        ...
        ##print('self attention input :',x.shape,'| output :',attn_output.shape)
        x = self.norm1(x + self.dropout(attn_output))
        attn_output = self.cross_attn(x, enc_output,enc_output,src_mask)
        ##print('cross attention encoder input :',enc_output.shape,'| dec input :',x.shape,'| output :',attn_output.shape)
...
class Transformer(nn.Module):
    ...
    def generate_mask(self, src, tgt):
        ...
        tgt_mask = tgt_mask.to(self.device) & nopeak_mask.to(self.device)
